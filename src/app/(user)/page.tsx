...
const newArrivalQuery = groq`*[_type == 'product' && position =='new-arrival']{
  _id, _rev, _createdAt, price, rowprice, title, position, ratings,
  description, 'brand': brand->title, slug, image,
  "category":category[0]->title, isnew, productId, body, quantity,
} | order(_createdAt asc)`;

const topDealQuery = groq`*[_type == 'product' && position =='top-deal']{
  _id, _rev, _createdAt, price, rowprice, title, position, ratings,
  description, 'brand': brand->title, slug, image,
  "category":category[0]->title, productId, isnew, body, quantity,
} | order(discount asc)`;
...
const categoryQuery = groq`*[_type == 'category']{ ... } | order(_createdAt asc)`;

const productsQuery = groq`*[_type == "brand"]{
  _id, title,
  "products": *[_type == "product" && references(^._id)]{
    _id, title, price, description, image, slug, position,
    ratings, 'brand': brand->title, "category":category[0]->title,
    rowprice, productId, isnew, _createdAt
  }
}`;
